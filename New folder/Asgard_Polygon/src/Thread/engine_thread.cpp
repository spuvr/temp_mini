...
void engine_thread()
{
	while (true)
	{
		// Updated GWorld offset
		game::GWorld = driver.read<uint64_t>(global->base + 0x8279f20);
		...
		// Updated team offset
		game::LocalPlayerTeam = driver.read<uint8_t>(game::LocalPlayerState + 0x428);
		...
		if (!NamePoolData)
		{
			...
			// Updated GNames offset
			driver.readm((void*)(global->base + 0x807a100), NamePoolData, sizeof(FNamePool));
		}
		...
		for (auto index{ 0 }; index < actor_array.Num(); index++)
		{
			...
			// Updated team offset
			auto team = driver.read<uint8_t>(playerstate + 0x428);

			auto health_comp = driver.read(actor + 0x730);
			if (!health_comp)
				continue;
			// Updated health offset
			...
			// Updated inventory offset
			auto inventory = driver.read(playerstate + 0x458);
